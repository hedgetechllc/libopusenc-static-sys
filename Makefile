...
docs:
	...
	echo "<meta http-equiv=\"refresh\" content=\"0;url=libopusenc_static_sys/index.html\" />" > target/doc/index.html

update:
	cargo update
	git submodule update
	git add libopusenc
	$(shell if git diff-index --quiet HEAD --; then git commit -m "libopusenc submodule updated" && git push; fi)
